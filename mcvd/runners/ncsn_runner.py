...
import mcvd.models.eval_models as eval_models

from mcvd.datasets import get_dataset, data_transform, inverse_data_transform
from mcvd.datasets.ffhq_tfrecords import FFHQ_TFRecordsDataLoader
from mcvd.evaluation.fid_PR import get_fid, get_fid_PR, get_stats_path, get_feats_path
from mcvd.losses import get_optimizer, warmup_lr
from mcvd.losses.dsm import anneal_dsm_score_estimation
...
from mcvd.models.fvd.fvd import get_fvd_feats, frechet_distance, load_i3d_pretrained
...
def get_model(config):
    ...
    if arch == 'unetmore':
        from mcvd.models.better.ncsnpp_more import UNetMore_DDPM # This lets the code run on CPU when 'unetmore' is not used
    ...
    def train(self):
        ...
        for epoch in range(start_epoch, self.config.training.n_epochs):
            for batch, (X, y) in enumerate(dataloader):
                ...
                # nb_logger.add_scalar('loss', loss, global_step=step)
                # tb_hook()
                ...
                if (step % self.config.training.snapshot_freq == 0 or step % self.config.training.sample_freq == 0) and self.config.training.snapshot_sampling:
                    ...
                    if conditional:
                        ...
                        condi = condi[:, :self.config.data.num_frames_cond*self.config.data.channels]
                        if future > 0:
                            futri = condi[:, self.config.data.num_frames_cond*self.config.data.channels:]
                        # Save gif
                        gif_frames = []
